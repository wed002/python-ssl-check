...
def lambda_handler(event, context):
    ...
    # Critical alerts 
      ...
      # First critical alert on 15th day      
        elif int(c) == 15:
            sns_Alert(dName, str(c), 'Critical')
    #third warning alert on 20th day      
        elif int(c) == 20:
            sns_Alert(dName, str(c), 'Warning')
    #second warning alert on 25th day
        elif int(c) == 25:
            sns_Alert(dName, str(c), 'Warning')
    #First warning alert on 30th day      
        elif int(c) == 30:
            sns_Alert(dName, str(c), 'Warning')
        else:
